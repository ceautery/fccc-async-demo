...
Elm.Inventory.make = function (_elm) {
   "use strict";
   _elm.Inventory = _elm.Inventory || {};
   if (_elm.Inventory.values) return _elm.Inventory.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var received = $Signal.mailbox("");
   var socket = A2($SocketIO.io,"http://localhost:3001",$SocketIO.defaultOptions);
   var eventName = "update_counts";
   var responses = Elm.Native.Task.make(_elm).perform(A2($Task.andThen,socket,A2($SocketIO.on,eventName,received.address)));
   var blankModel = {items: _U.list([]),errorMessage: ""};
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([$Html.text(model.errorMessage)
              ,$Basics.not($List.isEmpty(model.items)) ? A2($Html.div,
              _U.list([$Html$Attributes.$class("row header")]),
              _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text("Name")]))
                      ,A2($Html.div,_U.list([]),_U.list([$Html.text("Price")]))
                      ,A2($Html.div,_U.list([]),_U.list([$Html.text("In Stock")]))])) : A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,FetchData)]),
              _U.list([$Html.text("Load data")]))
              ,A2($Html.div,
              _U.list([]),
              A2($List.map,
              function (item) {
                 ...
                 _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text(item.name)]))
                         ,A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(item.price))]))
                         ,A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(item.num_stocked))]))]));
              },
              model.items))]));
   });
   var stringToAction = function (receivedString) {    return FetchData;};
   var inputs = _U.list([A2($Signal.map,stringToAction,received.signal)]);
   var Error = function (a) {    return {ctor: "Error",_0: a};};
   var resultToAction = function (result) {
      var _p0 = result;
      if (_p0.ctor === "Ok") {
            return DataFetched(_p0._0);
         } else {
            return Error($Basics.toString(_p0._0));
         }
   };
   var Item = F4(function (a,b,c,d) {    return {id: a,name: b,num_stocked: c,price: d};});
   var itemDecoder = $Json$Decode.list(A5($Json$Decode.object4,
   Item,
   A2($Json$Decode._op[":="],"id",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"name",$Json$Decode.string),
   A2($Json$Decode._op[":="],"num_stocked",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"price",$Json$Decode.$float)));
   var fetchData = $Effects.task(A2($Task.map,resultToAction,$Task.toResult(A3($Http.post,itemDecoder,"http://localhost:3000/fetchItems",$Http.empty))));
   var init = {ctor: "_Tuple2",_0: blankModel,_1: fetchData};
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "Error": return {ctor: "_Tuple2",_0: _U.update(model,{items: _U.list([]),errorMessage: _p1._0}),_1: $Effects.none};
         case "FetchData": return {ctor: "_Tuple2",_0: blankModel,_1: fetchData};
         default: return {ctor: "_Tuple2",_0: _U.update(model,{items: _p1._0}),_1: $Effects.none};}
   });
   var app = $StartApp.start({init: init,view: view,update: update,inputs: inputs});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var Model = F2(function (a,b) {    return {items: a,errorMessage: b};});
   return _elm.Inventory.values = {_op: _op
                                  ,Model: Model
                                  ,Item: Item
